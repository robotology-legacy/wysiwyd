...
#define CVZ_GUI_MODALITY_H 200
#define CVZ_GUI_MODALITY_W 200

static gboolean paintModalities(GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
	GdkPixbuf* pxBuf = (GdkPixbuf*)data;
	GdkPixbuf* scaledFrame = gdk_pixbuf_scale_simple(pxBuf, CVZ_GUI_MODALITY_W, CVZ_GUI_MODALITY_H, GDK_INTERP_BILINEAR); // Best quality
	...
	gdk_draw_rgb_image(widget->window,
		...
		0, 0,
		CVZ_GUI_MODALITY_W, CVZ_GUI_MODALITY_H,
...
public:
	...
	GtkWidget* allocate(IModality* m)
	{
		...
		gtk_box_pack_start(GTK_BOX(box), labelModInfo, FALSE, FALSE, 5);

		GtkWidget* boxModReal = gtk_hbox_new(FALSE, 5);
		...
		gtk_widget_set_size_request(frameReal, CVZ_GUI_MODALITY_W, CVZ_GUI_MODALITY_H);
		gtk_box_pack_start(GTK_BOX(boxModReal), frameReal, FALSE, FALSE, 0);
		gtk_widget_show(frameReal);

		GtkWidget* boxModPred = gtk_hbox_new(FALSE, 5);
		...
		gtk_widget_set_size_request(framePredicted, CVZ_GUI_MODALITY_W, CVZ_GUI_MODALITY_H);
