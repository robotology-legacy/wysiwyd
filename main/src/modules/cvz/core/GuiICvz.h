...
protected:
	...
	std::map<IModality*, GuiIModality* > wModalities;
	GtkWidget *label_name;
	GtkWidget *label_type;
	IConvergenceZone* myCvz;
	...
	virtual void initElements()
	{
		...
		* or if we return FALSE in the "delete-event" callback. */
		...
		for (std::map<std::string, IModality*>::iterator it = myCvz->modalitiesBottomUp.begin(); it != myCvz->modalitiesBottomUp.end(); it++)
		{
		...
		for (std::map<std::string, IModality*>::iterator it = myCvz->modalitiesTopDown.begin(); it != myCvz->modalitiesTopDown.end(); it++)
		{
			GuiIModality* mW = new GuiIModality();
			GtkWidget* boxMod = mW->allocate(it->second);
			wModalities[it->second] = mW;
...
public:
	...
	GuiICvz(IConvergenceZone* linkedCvz)
	{
		myCvz = linkedCvz;
	}
