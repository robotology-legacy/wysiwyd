/*
 * This merges two point clouds which have a different frame of reference  *
 */
...
PointCloudMerger::PointCloudMerger(ros::NodeHandle &nh, ros::NodeHandle &privNh) :
    pointcloudSubscriber1(nh,
                         privNh.param<std::string>("pc_from1", "/camera/depth_registered/points"),
                         1),
    pointcloudSubscriber2(nh,
                         privNh.param<std::string>("pc_from2", "/rtabmap/assembled_clouds"),
                         1),
    mergedPointcloudPublisher(nh.advertise<sensor_msgs::PointCloud2>(
                         privNh.param<std::string>("pc_to", "/merged_cloud"),
                         1)),
