...
% load iCub_motion_filtered.mat
% load baxter_cut_filtered.mat
...
%     x = [x;x_org_full(:,:,frm_idx+1);x_org_full(:,:,frm_idx+2)];
...
%             plot(x(p,1), x(p,2),'.','Color',color_idx(final(p)));
            plot(x_SVDD(p,1), x_SVDD(p,2),'.','Color','b');
            axis([0, width, 0, height]);
            hold on
        end
