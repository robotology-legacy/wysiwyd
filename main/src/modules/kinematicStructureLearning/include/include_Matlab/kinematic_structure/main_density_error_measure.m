...
filename_buf = {'puppet','iCub_motion.avi','iCub_hand.avi','robot_arm.mp4','baxter_cut.mp4','hand1.mp4'};
...
error_by_time_buf = cell(num_iter);   % for hand
...
for filename_idx = 1:size(filename_buf,2)
    for iter = 1:num_iter
        ...
        error_buf = [];
            ...
            %             error_by_time = cal_error_by_time(y, s, seg_idx, seg_center, seg_idx_GT, seg_center_GT);
            %             figure(286)
            %             plot(error_by_time);
            %             error_by_time_buf{iter} = error_by_time;
