...
%     frm_idx
    ...
    if SET.S.ndata ~= 0
        ...
        densityq(isnan(densityq)) = 0;
% %         densityq = densityq / sum(sum(densityq));
        densityq = densityq / max(max(densityq));
%         imagesc(densityq)
        max_rad_buf = max(max(sqrt(rad)));
        combination_buf = sqrt(rad) .* densityq;
        combination_buf = combination_buf / max(max(combination_buf));
        combination_buf = sqrt(combination_buf) * max_rad_buf;

%         densityq = densityq / max(max(densityq));
%         combination_buf = sqrt(rad) .* densityq;

        %---------------
        combination_buf(find(combination_buf < 10)) = 0;
        combination_buf(find(combination_buf >= 10)) = 255;
        img = uint8(combination_buf);
...
%         full_radius_map(:,:,frm_idx) = combination_buf;
        %=====================================================================        
        
        %%
        %-----------------------------------------------------------
        % drawing result
        if shape_SVDD_training_plot_ON
            h = figure(3);
            clf
