...
df1 <- read.table("topology/myGraph.txt", sep='\t', dec=',', header=T)
...
graphToLeda(g1, "comparisons/graph_alignment/MAGNA++_CLI_Linux64_01_30_2015/graph_P-leda-temp.gw", "comparisons/graph_alignment/MAGNA++_CLI_Linux64_01_30_2015/graph_P.gw")
graphToLeda(g2, "comparisons/graph_alignment/MAGNA++_CLI_Linux64_01_30_2015/graph_Q-leda-temp.gw", "comparisons/graph_alignment/MAGNA++_CLI_Linux64_01_30_2015/graph_Q.gw")
...
matrix2file(matrix.correspondance.g1constraint, "topology/matrix-correspondance-no-removal.output")
    ...
    for(i in 1:length(list.matrix.removeVertice[[V(g1)$name[v]]])){
        ...
        if(is.matrix(list.matrix.removeVertice[[V(g1)$name[v]]][i][[1]])){
            ...
            if(!is.matrix(list.matrix.g1.sub[[i]])){              
               list.matrix.g1.sub[[i]] = matrix((1/length(V(g1))), nrow = length(V(g1)), ncol = length(V(g2)))
...
matrix2file(matrix.correspondance.normalized, "topology/matrix-correspondance-with-removal.output")
