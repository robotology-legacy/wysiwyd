function img_output = genMatchImage_VideoSave_2DtoKinect(KineStruct_P, KineStruct_Q, X, method, pathname_P, pathname_Q)

fig_video_save_name = ['result/realSeq/kinectVideo/',KineStruct_P.videoFileName(1:end-4),'-Kinect-',method];
writerObj = VideoWriter([fig_video_save_name,'.avi']);
open(writerObj);

%%
slowValue = 3;
num_max_Frm = max(KineStruct_P.num_frames,KineStruct_Q.num_frames*slowValue);

frm_idx_set_P = ones(1,num_max_Frm)*(KineStruct_P.num_frames);
frm_idx_set_Q = ones(1,num_max_Frm)*(KineStruct_Q.num_frames)*slowValue;

frm_idx_set_P(1:KineStruct_P.num_frames) = [1:KineStruct_P.num_frames];
for i=1:slowValue
    frm_idx_set_Q(i:slowValue:slowValue*KineStruct_Q.num_frames) = [1:KineStruct_Q.num_frames];
end
...
for idx = 1:num_max_Frm
    ...
    % img_acc_Q = uint8(rgb2gray(imread([pathname_Q,'/',num2str(frm_idx_Q),'.png'])));
    img_acc_Q = uint8(imread([pathname_Q,'/',num2str(frm_idx_Q),'.png']));
    %     img_acc_P = uint8(read(xyloObj_P, frm_idx_P));
...
%     end
    ...
    for p = 1:size(X,1)
        for q = 1:size(X,2)
            if X(p,q) > 0
                hold on
                plot([KineStruct_P.seg_center(1,p,frm_idx_P) * ratio_width_P, shifted_dist + KineStruct_Q.seg_center_2D(1,q,frm_idx_Q) * ratio_width_Q],...
                    [KineStruct_P.seg_center(2,p,frm_idx_P) * ratio_height_P, KineStruct_Q.seg_center_2D(2,q,frm_idx_Q) * ratio_height_Q],'-','Color',[color_value,color_value,color_value],...
                    'LineWidth',3,...
                    'MarkerSize',10,...
                    'MarkerEdgeColor',[color_value,color_value,color_value],...
                    'MarkerFaceColor',[1.0,0.2,0.2]);
                plot([KineStruct_P.seg_center(1,p,frm_idx_P) * ratio_width_P, shifted_dist + KineStruct_Q.seg_center_2D(1,q,frm_idx_Q) * ratio_width_Q],...
                    [KineStruct_P.seg_center(2,p,frm_idx_P) * ratio_height_P, KineStruct_Q.seg_center_2D(2,q,frm_idx_Q) * ratio_height_Q],'-y.',...
                    'LineWidth',2,...
                    'MarkerSize',10,...
                    'MarkerEdgeColor','y',...
                    'MarkerFaceColor',[1.0,0.2,0.2]);
