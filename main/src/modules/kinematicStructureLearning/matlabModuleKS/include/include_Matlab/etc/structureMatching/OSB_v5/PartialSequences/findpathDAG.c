                         ...
                         double *pathcost, int **indxrow, int **indxcol )
{
    ...
    /* Initialize first row - remember, MatLab stores uses "column major" */
    for ( j = 0; j < n; j++ )
    {
        weight[j*m] = matx[j*m];
    }

    /* Initialize first column - remember, MatLab stores uses "column major" */
    for ( j = 0; j < m; j++ )
    {
        weight[j] = matx[j];
    }
    ...
    for ( i = 0; i < m-1; i++ )
    {
        for ( j = i; j < n-1; j++ ) /* i is starting value to force first element of query to participate in matching */
        {
            if ( fabs(i-j) <= warpwin )
            {
    ...
    *pathcost = INF;
    for ( j = 0; j < n-1; j++ )
    {
        if ( *pathcost > weight[(m-2) + ((j+1)*m)] )
        {
            *pathcost = weight[(m-2) + ((j+1)*m)];
            mincol = j+1;
        }
    }
