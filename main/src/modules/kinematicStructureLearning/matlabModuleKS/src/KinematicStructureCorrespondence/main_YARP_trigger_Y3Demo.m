...
% port2Interface = Port;
% port2Interface.close;
%
% disp('Creating a port to write to /kinematicStructureInterface/rpc');
% port2Interface.open('/matlab/kinematicStructure/InterfaceKS:o');

disp('=================================================================================================');
disp('- A. Kinematic structure extraction');
disp('>>>> Enter ''structureLearning #num_instance [left or right camera] [first frame] [last frame]''');
disp(' ');
disp('- B. Kinematic structure extraction & Joint Projection Test');
disp('>>>> Enter ''structureLearningToJoint #num_instance [left or right or kinect] [first frame] [last frame]''');
disp(' ');
disp('- C. Finding kinematic structure correspondences between two data');
disp('>>>> Enter ''findCorrespondence #num_instance [left or right or kinect] #num_instance [left or right or kinect]''');
disp(' ');
disp('- D. Finding kinematic structure correspondences between two body parts and an object');
disp('>>>> Enter ''findCorrespondenceWithObject #num_instance [left or right or kinect] #num_instance [left or right or kinect]''');
disp(' ');
...
while(~shouldClose)
    ...
    if (all(isletter(bCommand.get(0))))
        if (strcmp(bCommand.get(0).toString, 'quit') || strcmp(bCommand.get(0).toString, 'close'))
            ...
            %----------------------------------------------------------------------
        elseif (strcmp(bCommand.get(0).toString, 'structureLearning'))     % kinematic structure learning from one video
            %%
            %=========================================================
            % YARP instruction
            %=========================================================
            ...
            %----------------------------------------------------------------------
        elseif (strcmp(bCommand.get(0).toString, 'structureLearningToJoint'))     % Show Motor Joint Projection
            %%
            %=========================================================
            % YARP instruction
            %=========================================================
            ...
            % ---- saving feature values
            %=========================================================
            submodule_video_loading_feature_extraction_jointProjection;
            ...
            KineStruct = submodule_JointProjection2SegJoint(KineStruct);
        ...
        elseif (strcmp(bCommand.get(0).toString, 'findCorrespondence'))
            ...
            %----------------------------------------------------------------------
        elseif (strcmp(bCommand.get(0).toString, 'findCorrespondenceWithObject'))
            %% kinematic structure correspondences matching from two videos
            %=========================================================
            % YARP instruction
            %=========================================================
            ...
            %                 wrapper_YARP_ABM_retrieve_for_Correspondence;
%             wrapper_YARP_ABM_retrieve_for_Correspondence_PortByPort
                            wrapper_YARP_ABM_retrieve_for_Correspondence_PortByPort_Object;
            ...
            % ------ converting feature data to y
            % 2. Kinematic structure generation
            % 3. Add object location to KineStruct
            %=========================================================
            if strcmp(data_source_P,'left') || strcmp(data_source_P,'right')
                idx = 'P';
                cdata_P = featureExtractionYARP(idx);
            ...
            if HGM_method <= 3
                                    problem = createSimilarity_RRWHM_relative_object(KineStruct_P, KineStruct_Q);
%                 problem = createSimilarity_RRWHM_relative(KineStruct_P, KineStruct_Q);
            elseif HGM_method == 4 || HGM_method == 5 || HGM_method == 6
                problem = createSimilarity_BCAGM(KineStruct_P, KineStruct_Q);
            end
            ...
            %=========================================================
            %                 img_output = genMatchImages(cdata_P, cdata_Q, KineStruct_P, KineStruct_Q, data_source_P, data_source_Q, X, 'PROPOSED_RRWHM');
            img_output = genMatchImages_object(cdata_P, cdata_Q, KineStruct_P, KineStruct_Q, data_source_P, data_source_Q, X, 'PROPOSED_RRWHM');
            %                 img_output = genMatchImages_object_kinect2kinect(cdata_P, cdata_Q, KineStruct_P, KineStruct_Q, data_source_P, data_source_Q, X, 'PROPOSED_RRWHM');
            %                 img_output = genMatchImageSeq(cdata_P, cdata_Q, KineStruct_P, KineStruct_Q, data_source_P, data_source_Q, X, 'PROPOSED_RRWHM');
            ...
            if strcmp(data_source_P,'left') || strcmp(data_source_P,'right')
                KineStruct_P = submodule_JointProjection2SegJoint(KineStruct_P);
            elseif strcmp(data_source_P,'kinect')
