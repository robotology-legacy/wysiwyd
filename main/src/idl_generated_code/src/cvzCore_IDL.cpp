...
class cvzCore_IDL_start : public yarp::os::Portable {
public:
...
class cvzCore_IDL_pause : public yarp::os::Portable {
public:
  virtual bool write(yarp::os::ConnectionWriter& connection) {
    yarp::os::idl::WireWriter writer(connection);
    if (!writer.writeListHeader(1)) return false;
...
class cvzCore_IDL_quit : public yarp::os::Portable {
public:
  bool _return;
  virtual bool write(yarp::os::ConnectionWriter& connection) {
    yarp::os::idl::WireWriter writer(connection);
...
void cvzCore_IDL::start() {
  cvzCore_IDL_start helper;
  if (!yarp().canWrite()) {
    fprintf(stderr,"Missing server method '%s'?\n","void cvzCore_IDL::start()");
  }
  yarp().write(helper,helper);
}
void cvzCore_IDL::pause() {
  cvzCore_IDL_pause helper;
  if (!yarp().canWrite()) {
    fprintf(stderr,"Missing server method '%s'?\n","void cvzCore_IDL::pause()");
  }
  yarp().write(helper,helper);
}
bool cvzCore_IDL::quit() {
  bool _return = false;
  cvzCore_IDL_quit helper;
  if (!yarp().canWrite()) {
    fprintf(stderr,"Missing server method '%s'?\n","bool cvzCore_IDL::quit()");
  }
...
bool cvzCore_IDL::read(yarp::os::ConnectionReader& connection) {
  yarp::os::idl::WireReader reader(connection);
  reader.expectAccept();
...
std::vector<std::string> cvzCore_IDL::help(const std::string& functionName) {
